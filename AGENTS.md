...
- Podman scripts (`build.sh`, `test.sh`, `reinstall.sh`) mirror the Dockerfile stack (Temurin JDK 17 + SDK 34). Update scripts plus README.md/CLAUDE.md/AGENTS.md together whenever workflows change.
...
- `./reinstall.sh` — uninstall/install latest APK via adb for manual QA.
...
## Commit & PR Workflow
- Keep commits short, imperative, and single-purpose (`Fix gradle cache in Dockerfile`). Describe non-trivial verification commands in the body and rerun `./build.sh` or `./gradlew check` afterward.
